...
for dtype in dtypes:
    ...
    print('\narray:\n', a)
    check('square root', np.sqrt(a, dtype=outtype), [math.sqrt(x) for x in range(8)], outtype)
    print('\narray:\n', b)
    check('square root', np.sqrt(b, dtype=outtype), [math.sqrt(x) for x in range(8)], outtype)
